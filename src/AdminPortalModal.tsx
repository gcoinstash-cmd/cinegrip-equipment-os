...
export const AdminPortalModal: React.FC<AdminPortalModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (passcode.toLowerCase() === 'cinegrip2026') {
      ...
      <div className="relative w-full max-w-4xl bg-[#121214] border border-rose-500/30 rounded-2xl p-6 sm:p-8 shadow-2xl text-zinc-100 max-h-[90vh] overflow-y-auto">
        ...
        {!isAuthenticated ? (
          <div className="py-8 max-w-md mx-auto text-center">
            ...
            <h3 className="text-2xl font-bold tracking-tight mb-2">CINEGRIP EQUIPMENT OS Portal</h3>
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              <div>
                <input
                  ...
                  placeholder="Enter manager passkey (cinegrip2026)"
              ...
              {error && (
                <p className="text-red-400 text-xs font-mono">Invalid passkey. Use: cinegrip2026</p>
              )}
          ...
          <div>
            <div className="flex flex-wrap items-center justify-between gap-4 border-b border-zinc-800 pb-6 mb-6">
              <div>
                ...
                <h3 className="text-xl font-bold text-white">CINEGRIP EQUIPMENT OS // BACK-OFFICE</h3>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 rounded-xl bg-zinc-900/60 border border-zinc-800">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                      ...
                      <div key="gear_inventory" className="p-2.5 rounded bg-zinc-950 border border-zinc-800 text-xs font-mono text-zinc-300 flex items-center gap-1.5">
                        <CheckCircle className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" />
                        <span>gear_inventory</span>
                      </div>
                    
                      <div key="production_rentals" className="p-2.5 rounded bg-zinc-950 border border-zinc-800 text-xs font-mono text-zinc-300 flex items-center gap-1.5">
                        <CheckCircle className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" />
                        <span>production_rentals</span>
                      </div>
                    
                      <div key="insurance_binders" className="p-2.5 rounded bg-zinc-950 border border-zinc-800 text-xs font-mono text-zinc-300 flex items-center gap-1.5">
                        <CheckCircle className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" />
                        <span>insurance_binders</span>
                      </div>
                    
                      <div key="subrental_logs" className="p-2.5 rounded bg-zinc-950 border border-zinc-800 text-xs font-mono text-zinc-300 flex items-center gap-1.5">
                        <CheckCircle className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" />
                        <span>subrental_logs</span>
                      </div>
            ...
            {activeTab === 'telemetry' && (
              <div className="p-4 rounded-xl bg-zinc-900 border border-zinc-800 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div key="ACTIVE RENTAL PACKAGES" className="p-3 rounded-lg bg-zinc-950 border border-zinc-800">
                      <span className="text-xs text-zinc-300 font-mono">ACTIVE RENTAL PACKAGES</span>
                      <p className="text-lg font-bold text-rose-400 font-mono mt-0.5">{"280 PACKAGES"}</p>
                    </div>
                  
                    <div key="INSURANCE VERIFICATION" className="p-3 rounded-lg bg-zinc-950 border border-zinc-800">
                      <span className="text-xs text-zinc-300 font-mono">INSURANCE VERIFICATION</span>
                      <p className="text-lg font-bold text-rose-400 font-mono mt-0.5">{"100% PRE-CHECKED"}</p>
                    </div>
                  
                    <div key="SUB-RENTAL REVENUE MARGIN" className="p-3 rounded-lg bg-zinc-950 border border-zinc-800">
                      <span className="text-xs text-zinc-300 font-mono">SUB-RENTAL REVENUE MARGIN</span>
                      <p className="text-lg font-bold text-rose-400 font-mono mt-0.5">{"42.5%"}</p>
                    </div>
                  
                    <div key="EQUIPMENT READINESS" className="p-3 rounded-lg bg-zinc-950 border border-zinc-800">
                      <span className="text-xs text-zinc-300 font-mono">EQUIPMENT READINESS</span>
                      <p className="text-lg font-bold text-rose-400 font-mono mt-0.5">{"99.8% PREPPED"}</p>
                    </div>
            ...
            {activeTab === 'sql' && (
              <div className="p-4 rounded-xl bg-zinc-950 border border-zinc-800 font-mono text-xs text-zinc-300 overflow-x-auto">
                ...
                <div className="text-rose-400">ALTER TABLE gear_inventory ENABLE ROW LEVEL SECURITY;</div>
                <div className="text-zinc-400 mt-1">CREATE POLICY "Allow authenticated read" ON gear_inventory FOR SELECT USING (true);</div>
